...
def mask_poly(frame,poly_points):
    ...
    img = cv2.fillPoly(img,[poly_points],(0,0,0))
    cv2.namedWindow('post_mask',cv2.WINDOW_NORMAL)
    cv2.imshow('post_mask',img)
...
def image_preprocessing(img):
	...
	cv2.imshow('color ', color_blend)
	# Project the image back to the orignal coordinates
	newwarp = cv2.warpPerspective(color_blend, Hinv, (crop_img.shape[1], crop_img.shape[0]))
...
# Read the frames
cap = cv2.VideoCapture('media/problem_3.mp4')
